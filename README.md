...
VLESS Reality        → VPS за рубежом → Интернет     ← основной
Yandex Cloud Relay   → VPS за рубежом → Интернет     ← обход белых списков
WebRTC / Телемост    → VPS за рубежом → Интернет     ← аварийный
Cloudflare CDN       → VPS за рубежом → Интернет     ← backup для Wi-Fi
```

| Метод | Когда нужен | Платформы |
|-------|-------------|-----------|
| **VLESS Reality** | Всегда, основной вариант | Все |
| **Yandex Cloud Relay** | Мобильная сеть с белыми списками, IP VPS заблокирован | Все |
| **WebRTC / Телемост** | Полная блокировка, ничего не работает | Windows, Linux (через OlcRTC -- пока нет мобильных клиентов) |
| **Cloudflare CDN** | Backup при блокировке IP VPS (только домашний Wi-Fi, т.к. Cloudflare блокируется ТСПУ с 2025) | Все |
