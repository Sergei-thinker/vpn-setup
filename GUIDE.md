...
| 1 | Cloudflare CDN (WebSocket) | VPS IP заблокирован (только Wi-Fi!) | Средняя | +$6/год (первый год ~$3) (домен) |
...
| Домен (для CDN) | ~$6/год (первый год ~$3) | Рекомендуется |
...
|-----------|------|-------|--------|
| **Aeza** (aeza.net) | от 1.99 EUR/мес | Самый дешёвый, почасовая оплата, быстрая смена IP | Нет техподдержки на промо-тарифе |
...
> **Не используйте:** Hetzner, OVH, DigitalOcean, AWS -- их подсети массово заблокированы ТСПУ.
...
Любой дешёвый домен. Namecheap -- от ~$6/год (первый год ~$3). Домен нужен для Cloudflare CDN-фронтинга.
...
Рассмотрим на примере Aeza (самый бюджетный вариант):

1. Зайдите на [aeza.net](https://aeza.net/)
2. Зарегистрируйтесь, пополните баланс (карта, крипто)
3. Создайте сервер:
   - Тариф: **SWE-PROMO** (или аналогичный)
...
2. Купите самый дешёвый домен (~$6/год, первый год ~$3). Например: `mysite123.top`
...
Запишите **Path** (например `/ws-vless-YOUR_SECRET_PATH`) -- он понадобится для Worker.
...
- С CDN: +$6/год (первый год ~$3) (домен)
...
| [Aeza](https://aeza.net/) | Бюджетные VPS для VPN |
