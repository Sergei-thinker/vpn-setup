...
> **4-уровневая система на базе VLESS Reality + Russian Relay VPS + WebRTC + Cloudflare CDN.**
...
5. [Layer 1: Relay через российский VPS](#5-layer-1-relay-через-российский-vps)
6. [Layer 2: WebRTC через Яндекс.Телемост (аварийный)](#6-layer-2-webrtc-через-яндекстелемост-аварийный)
7. [Layer 3: Cloudflare CDN (Domain Fronting)](#7-layer-3-cloudflare-cdn-domain-fronting)
...
|                              Google, Apple                          |
|                                                                     |
|  Layer 1: RELAY ЧЕРЕЗ РОССИЙСКИЙ VPS (для обхода белых списков)     |
|  +----------+  VLESS  +-----------+  xHTTP  +----------+           |
|  |Устройство| ------> |RU VPS (РФ)| ------> | VPS SWE  |->Net     |
|  +----------+         |SNI:gos.ru |          +----------+           |
|                       +-----------+                                 |
|  Timeweb / VDSina / Selectel. NB: YC НЕ в белых списках (см. ниже). |
|                                                                     |
...
|                                                                     |
|  Layer 3: CDN-ФРОНТИНГ ЧЕРЕЗ CLOUDFLARE                            |
|  +----------+   HTTPS/WSS    +-----------+      +----------+       |
...
| 1 | Relay через российский VPS (Timeweb/VDSina) | **Белые списки на мобильной сети** | Средняя | +~80-100 RUB/мес (~$1) |
| 2 | WebRTC через Яндекс.Телемост | Полный white-list, всё заблокировано | Высокая | Бесплатно |
| 3 | Cloudflare CDN (WebSocket) | VPS IP заблокирован (только Wi-Fi!) | Средняя | +1 EUR/год (домен) |

> **Обновление (апрель 2026):** Cloudflare **активно блокируется ТСПУ с середины 2025**. Layer 3 работает только на домашнем Wi-Fi. Для мобильной сети с белыми списками используйте **Layer 1** — relay на российском VPS. **NB:** Yandex Cloud не обходит белые списки (AS Yandex.Cloud LLC ≠ AS YANDEX LLC, по критике в комментах к [Habr 1021160](https://habr.com/ru/articles/1021160/)).
...
4. **RU-Relay** через российский VPS (Timeweb/VDSina) -- **при белых списках на мобильной сети**
5. WebRTC через Яндекс.Телемост -- последний рубеж (только десктоп)
6. `ws-cloudflare` через Cloudflare CDN -- если IP VPS заблокирован (только Wi-Fi!)
...
| RU relay VPS (Timeweb/VDSina) | ~80-100 RUB/мес (~$1) | Для обхода белых списков |
...
## 5. Layer 1: Relay через российский VPS

### 5.1. Зачем нужен relay

Когда мобильные операторы (МТС, Мегафон, Tele2, Beeline) включают **белые списки**, пропускается только трафик к российским IP. Cloudflare тоже блокируется с 2025 года. Relay на российском VPS решает проблему:

```
Устройство --> RU VPS (Timeweb/VDSina) --> VPS за рубежом --> Интернет
               VLESS Reality                xHTTP relay
               (SNI: gosuslugi.ru)          Порт 10443
```

> **⚠️ Yandex Cloud НЕ использовать.** Ранее проект предлагал relay через YC preemptible VM с расчётом «IP YC в белых списках ТСПУ». По критике `@paxlo`, `@aax`, `@Varpun` в комментах к [Habr 1021160](https://habr.com/ru/articles/1021160/) тезис опровергнут: AS `Yandex.Cloud LLC` (пользовательские VM) и AS `YANDEX LLC` (сервисы Яндекса) — разные автономные системы, ТСПУ фильтрует их раздельно. YC-VM блокируется при активных белых списках. Скрипты `deploy-relay-yc.sh`/`rotate-relay-yc.sh`/`yc-cloud-init.yaml.tpl` удалены 2026-04-17.

### 5.2. Что понадобится

| Компонент | Стоимость | Комментарий |
|-----------|-----------|-------------|
| Российский VPS (Timeweb Cloud) | ~80 RUB/мес (~$1) | Выбирайте минимальный тариф |
| Российский VPS (VDSina) | ~100 RUB/мес (~$1.2) | Альтернатива Timeweb |
| Российский VPS (Selectel / VK Cloud) | ~100-200 RUB/мес | Другие варианты |
| SSH-доступ | — | Обычно root с паролем или ключом |

**IP в белых списках не гарантирован** ни у одного из провайдеров. Нужно проверять после деплоя. Если IP не проходит — пересоздать VPS или выбрать другого провайдера.

### 5.3. Подготовка основного VPS (принимающая сторона)

На основном зарубежном VPS нужно создать inbound для приёма relay-трафика.
...
RELAY_HOST=<IP российского VPS>
RELAY_SSH_PORT=22
RELAY_SSH_USER=root
RELAY_SSH_KEY=~/.ssh/id_ed25519
RELAY_PROVIDER=generic

SWEDEN_VPS_IP=<IP основного зарубежного VPS>
SWEDEN_RELAY_PORT=10443
SWEDEN_RELAY_UUID=<UUID из шага 5.3>
SWEDEN_RELAY_PUBKEY=<Public Key из шага 5.3>
SWEDEN_RELAY_SID=<Short ID из шага 5.3>
```
...
bash deploy-relay.sh \
  --sweden-ip "$SWEDEN_VPS_IP" \
  --sweden-port "$SWEDEN_RELAY_PORT" \
  --sweden-uuid "$SWEDEN_RELAY_UUID" \
  --sweden-pubkey "$SWEDEN_RELAY_PUBKEY" \
  --sweden-sid "$SWEDEN_RELAY_SID"
```

Скрипт автоматически:
1. Подключается по SSH к RU VPS
2. Устанавливает Xray-core
3. Генерирует x25519 ключи и UUID для relay inbound
4. Создаёт конфиг: inbound VLESS Reality на 443 → outbound VLESS xHTTP Reality на основной VPS:10443
5. Настраивает firewall и автозапуск
6. Выводит **VLESS URI** для клиента
7. Сохраняет credentials в `/root/relay-credentials.txt` на relay VPS

### 5.6. Настройка клиента для relay

После `deploy-relay.sh` вы получите VLESS URI. Импортируйте его в клиент, или настройте вручную:

| Поле | Значение |
|------|----------|
| Protocol | VLESS |
| Address | IP российского VPS |
| Port | **443** |
| UUID | (из relay-credentials.txt) |
| Transport | TCP |
| Security | Reality |
| SNI | **www.gosuslugi.ru** |
| Fingerprint | `chrome` |
| Public Key | (из relay-credentials.txt) |
| Short ID | (из relay-credentials.txt) |
| Flow | пусто |

### 5.7. Тестирование

1. Добавьте VLESS URI в клиент (Hiddify/Shadowrocket/v2rayNG/v2rayN)
2. Подключитесь через профиль `Relay-Russia`
3. Зайдите на [2ip.ru](https://2ip.ru/) -- должен показать IP **основного зарубежного** VPS (не российского!)
4. Если показывает российский IP — значит outbound не отработал, смотрите логи: `python ssh_exec.py -t relay logs`

### 5.8. Мониторинг и управление
...
### 5.9. Fallback при блокировке

```
RU VPS заблокирован → пересоздать у другого провайдера (VDSina → Timeweb → Selectel)
Все российские VPS   → Layer 2 (WebRTC через Яндекс.Телемост)
```

> **Предупреждение:** Минцифры предупредило крупных российских провайдеров о необходимости блокировать VPN-пользователей. Имейте запасного провайдера и готовый скрипт для быстрого пересоздания relay.

---

## 6. Layer 2: WebRTC через Яндекс.Телемост (аварийный)
...
> **Важно:** Layer 2 работает **только на десктопе**. Для мобильных устройств Layer 1 (relay) -- последний рубеж.

### 6.2. Установка OlcRTC
...
## 7. Layer 3: Cloudflare CDN (Domain Fronting)

### 7.1. Зачем нужен CDN

> **Важное обновление (апрель 2026):** Cloudflare **активно блокируется ТСПУ с середины 2025**. При белых списках на мобильной сети Layer 3 **не работает**. Для обхода белых списков на мобильном интернете используйте **Layer 1 (relay на российском VPS)**.
>
> Layer 3 остаётся полезным на **домашнем Wi-Fi**, где блокировка менее агрессивна.
...
### 7.4. Создание WebSocket inbound на VPS
...
5. Если ни один порт не работает (IP заблокирован):
   -> На мобильном: Relay на российском VPS (Layer 1)
   -> На Wi-Fi: Cloudflare CDN (Layer 3)
                    |
                    v
6. Если CDN и Relay не работают:
   -> WebRTC через Яндекс.Телемост (Layer 2, только десктоп)
                    |
                    v
7. Крайний случай:
...
- С RU relay VPS: +~1 EUR/мес (Timeweb ~80 RUB/мес, VDSina ~100 RUB/мес)
- Итого максимум: ~5-6 EUR/мес
...
- **Layer 1 (relay на российском VPS — Timeweb/VDSina):** ✅ **основной метод** — шанс пройти белые списки; требует проверки IP после деплоя
- **Layer 2 (WebRTC):** Яндекс.Телемост всегда в белом списке (но только десктоп)
- **Layer 3 (Cloudflare):** ⚠ **не работает** при белых списках (CF блокируется с 2025)

Для обхода белых списков на мобильной сети используйте **Layer 1** (раздел 5).
...
| [Timeweb Cloud](https://timeweb.cloud/) | Российский VPS для relay (~80 RUB/мес) |
| [VDSina](https://vdsina.com/) | Российский VPS для relay (~100 RUB/мес) |
| [Selectel](https://selectel.ru/) | Российский VPS для relay |
...
- **Layer 1** (relay на российском VPS — Timeweb/VDSina) -- **при белых списках на мобильной сети**
- **Layer 2** (WebRTC) -- аварийный, последний рубеж (только десктоп)
- **Layer 3** (Cloudflare CDN) -- если IP VPS заблокирован (домашний Wi-Fi)

> **Ключевое обновление 2026:** Cloudflare блокируется ТСПУ. Для мобильной сети Layer 1 (relay на российском VPS) стал основным резервным методом. **NB:** Yandex Cloud удалён из проекта 2026-04-17 — тезис «IP YC в белых списках» опровергнут в комментах к [Habr 1021160](https://habr.com/ru/articles/1021160/).

Общая стоимость: **~2 EUR/мес** (базовый), **~3 EUR/мес** (с relay на RU VPS). Время настройки с нуля: **2-4 часа** (Layer 0 + 3). Время деплоя Layer 1: **~10 минут** (скрипт `deploy-relay.sh`). Время восстановления при блокировке IP: **~30 минут** с подготовленными скриптами.
