...
> **4-уровневая система на базе VLESS Reality + Yandex Cloud Relay + WebRTC + Cloudflare CDN.**
...
5. [Layer 1: Relay через Yandex Cloud](#5-layer-1-relay-через-yandex-cloud)
...
|                              Google, Apple                          |
|                                                                     |
|  Layer 1: RELAY ЧЕРЕЗ YANDEX CLOUD (рекомендуемый для мобильных)    |
|  +----------+  VLESS  +----------+  xHTTP  +----------+            |
...
| 1 | Relay через Yandex Cloud | **Белые списки на мобильной сети** | Средняя | +~400 RUB/мес (~4 EUR) |
...
> **Обновление (апрель 2026):** Cloudflare **активно блокируется ТСПУ с середины 2025**. Layer 3 работает только на домашнем Wi-Fi. Для мобильной сети с белыми списками используйте **Layer 1 (Yandex Cloud)** -- IP Yandex Cloud подтверждённо в белых списках.
...
4. **YC-Relay** через Yandex Cloud -- **при белых списках на мобильной сети**
...
## 5. Layer 1: Relay через Yandex Cloud
...
### 5.3. Подготовка шведского VPS (принимающая сторона)
...
### 5.7. Anti-detection (маскировка от SmartWebSecurity)
...
### 5.8. Настройка клиента для relay
...
### 5.9. Preemptible VM: авто-рестарт
...
> **Важное обновление (апрель 2026):** Cloudflare **активно блокируется ТСПУ с середины 2025**. При белых списках на мобильной сети Layer 3 **не работает**. Для обхода белых списков на мобильном интернете используйте **Layer 1 (Yandex Cloud relay)**.
>
...
5. Если ни один порт не работает (IP заблокирован):
   -> На мобильном: Yandex Cloud Relay (Layer 1)
...
На сайте показана обычная VM с полной мощностью CPU. Мы экономим двумя способами: **прерываемая VM** (в ~3 раза дешевле, перезапускается раз в сутки) и **20% мощности CPU** (relay не нужен полный процессор). Подробная разбивка — в [секции 5.2](#52-что-понадобится).
...
- **Layer 1 (Yandex Cloud relay):** ✅ **основной метод** -- IP Yandex Cloud в белых списках
...
- **Layer 1** (Yandex Cloud relay) -- **при белых списках на мобильной сети**
- **Layer 2** (WebRTC) -- аварийный, последний рубеж (только десктоп)
- **Layer 3** (Cloudflare CDN) -- если IP VPS заблокирован (домашний Wi-Fi)

> **Ключевое обновление 2026:** Cloudflare блокируется ТСПУ. Для мобильной сети Layer 1 (Yandex Cloud) стал основным резервным методом.

Общая стоимость: **~2 EUR/мес** (базовый), **~6 EUR/мес** (с relay). Время настройки с нуля: **2-4 часа** (Layer 0 + 3). Время деплоя Layer 1: **~10 минут** (скрипт `deploy-relay-yc.sh`). Время восстановления при блокировке IP: **~30 минут** с подготовленными скриптами.
